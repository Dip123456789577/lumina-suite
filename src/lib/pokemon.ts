...
export async function fetchPokemon(idOrName: string | number): Promise<PokemonDetail> {
  ...
  const flavor =
    (species.flavor_text_entries as { flavor_text: string; language: { name: string } }[])
      .find((e) => e.language.name === "en")
      ?.flavor_text.replace(/[\f\n\r]/g, " ") ?? "";
  const genus =
    (species.genera as { genus: string; language: { name: string } }[]).find(
  ...
  const moves = (data.moves as { move: { name: string; url: string } }[]).slice(0, 8).map((m) => ({
    name: m.move.name.replace(/-/g, " "),
    type: "normal" as PokeType,
    power: null,
    accuracy: null,
  }));
