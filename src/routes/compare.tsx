...
function Compare() {
  ...
  const { data: pa } = useQuery({
    queryKey: ["pokemon", a],
    queryFn: () => fetchPokemon(a),
    staleTime: Infinity,
  });
  const { data: pb } = useQuery({
    queryKey: ["pokemon", b],
    queryFn: () => fetchPokemon(b),
    staleTime: Infinity,
  });
...
function Slot({
  ...
  return (
    <div
      className={`glass-strong rounded-3xl p-6 ${side === "left" ? "md:text-right" : "md:text-left"}`}
    >
      <div className="label-caps text-foreground/60">
        {side === "left" ? "Specimen A" : "Specimen B"}
      </div>
      <div className="mt-3 flex gap-2">
        ...
        />
        <MagneticButton onClick={() => onChange(draft)} variant="ghost">
          Scan
        </MagneticButton>
      </div>
    </div>
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="relative h-32 w-32 shrink-0">
          <div
            ...
            style={{ background: tcol.glow, opacity: 0.6 }}
          />
        ...
        {(["hp", "attack", "defense", "spAtk", "spDef", "speed"] as const).map((k, i) => (
          <div key={k}>
            <div className="flex justify-between text-xs label-caps text-foreground/60">
              <span>{k}</span>
              <span className="font-mono text-foreground">{p.stats[k]}</span>
            </div>
            <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-white/5">
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(100, (p.stats[k] / 180) * 100)}%` }}
